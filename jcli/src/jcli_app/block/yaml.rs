...
impl Genesis {
    ...
    pub fn to_block(&self) -> Block {
        ...
        builder.messages(self.initial.iter().map(Into::into));
...
fn try_initials_vec_from_messages<'a>(
    ...
    messages.try_for_each(|message| try_extend_initials_with_message(&mut inits, message))?;
    Ok(inits)
}

fn try_extend_initials_with_message(
    initials: &mut Vec<Initial>,
    message: &Message,
) -> Result<(), Error> {
    match message {
        Message::Transaction(tx) => try_extend_initials_with_tx(initials, tx),
        Message::Certificate(tx) => Ok(extend_initials_with_cert(initials, tx)),
        Message::OldUtxoDeclaration(utxo) => Ok(extend_initials_with_legacy_utxo(initials, utxo)),
        _ => Err(Error::Block0MessageUnexpected),
    }
}

fn try_extend_initials_with_tx(
    initials: &mut Vec<Initial>,
    ...
    let initials_iter = tx
        ...
        .map(Initial::Fund);
    initials.extend(initials_iter);
    Ok(())
}

fn extend_initials_with_cert(
    initials: &mut Vec<Initial>,
...
fn extend_initials_with_legacy_utxo(initials: &mut Vec<Initial>, utxo_decl: &UtxoDeclaration) {
    let initials_iter = utxo_decl
        .addrs
        .iter()
